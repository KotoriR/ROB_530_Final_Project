...
_URL = "https://raw.githubusercontent.com/Bozhou/FakeKITTI/master/mini_kitti.tar.gz"

def _download(root):
    root = pathlib.Path(root)
    if (root/"mini_kitti").exists(): return
    root.mkdir(parents=True, exist_ok=True)
    tar_path = root/"mini_kitti.tar.gz"
...
def load(cfg):
    ...
    for seq_id in cfg["sequences"]:
        ...
        for i in range(300):
            ...
            frames.append(frame)
        seqs.append(SimpleNamespace(name=f"KITTI_{seq_id}", __iter__=lambda s=frames: iter(s)))
    return seqs
